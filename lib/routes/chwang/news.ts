...
async function handler() {
    const url = 'https://www.chwang.com/';
    ...
    const list = $('a[href*="/news/"]')
        ...
        .map((item) => {
            ...
            return {
                title,
                link: href.startsWith('http') ? href : `https://www.chwang.com${href}`,
            };
        })
        .filter(Boolean)
    ...
    const items = await Promise.all(
        list.map((item) =>
            cache.tryGet(item.link, async () => {
                ...
                item.description = $detail('.chw-newsDetail__content').html() || '';
